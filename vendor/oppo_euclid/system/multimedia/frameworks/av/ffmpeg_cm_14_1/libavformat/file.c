...
#endif
#ifdef VENDOR_EDIT
//XingLong.Tang@PSW.MM.MediaServer.Player, 2017/10/29, Add for integrate ffmpeg
#define SHAREFD 0x0080
#endif /* VENDOR_EDIT */
...
#ifdef VENDOR_EDIT
//XingLong.Tang@PSW.MM.MediaServer.Player, 2017/10/29, Add for integrate ffmpeg
    if (strstr(filename, "sharedfd://")) {
        h->flags |= SHAREFD;
        sscanf(filename, "sharedfd://%d:%d:%d", &fd, &offset, &length);
    }
    if (fd < 0)
#endif /* VENDOR_EDIT */
