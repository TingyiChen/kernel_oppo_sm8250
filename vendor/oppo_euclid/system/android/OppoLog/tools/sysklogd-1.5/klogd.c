...
#endif
*/
static char	*PidFile = "/dev/log/klogd.pid";
#endif
...
static int current_log_filenum = 0;
static char log_filename[200];

/* mode 0 = empty open file
              1  = append file
*/
static void ChangeNextLogFile(int mode)
{
	int base_len = 0;
	FILE *log_file = (FILE *) 0;
	int tmp_filenum;

	// first close ori file
	if ( output_file != (FILE *) 0 )
		fclose(output_file);

	// get filenumber
	current_log_filenum ++;
	if(current_log_filenum >= LOG_FILE_NUMBER)
	{
		current_log_filenum = 0;
	}
	
	base_len = strlen(log_filename) - 5;
	log_filename[base_len] = current_log_filenum + '0';

	// open file
	if(!mode)
	{
		output_file = fopen(log_filename, "w");
		if(output_file == (FILE *) 0 )
		{
			// error
			return;
	...
	{
		output_file = fopen(log_filename, "a+");
		if(output_file == (FILE *) 0 )
		{
			// error
			return;
	...
	// empty next logfile
	tmp_filenum = current_log_filenum;
	tmp_filenum ++;
	if(tmp_filenum >= LOG_FILE_NUMBER)
	{
		tmp_filenum = 0;
	}
	log_filename[base_len] = tmp_filenum + '0';

	/* need empty file content */
...
static void GetLogFile(void)
{
	...
	while(1)
	{
		...
		if(log_file == (FILE *) 0 )
		{
			// file not found. used this file
			// TODO :
			if(current_log_filenum > 0)
				current_log_filenum --;
			else
				current_log_filenum = LOG_FILE_NUMBER;
			ChangeNextLogFile(0);
			return;
		}
		
		fseek(log_file, 0, SEEK_END);
		if(ftell(log_file) < LOG_FILE_SIZE)
		{
			// used this file 
			// TODO :
			fclose(log_file);
...

{
	...
	if ( terminate == 1 )
	{
		Syslog(LOG_INFO,"ChangeLogging terminate == 1  terminate \n");
		Terminate();
	}
...

{
	...
	if ( output_file != (FILE *) 0 )
	{
		...
		{
			msg_level = argl[1] - '0';
			if( msg_level > record_log_level)
			{
				va_end(ap);
				if ( debugging )
				{
					fprintf(stderr, "klogd: %d %c\n", record_log_level, argl[1]);
				}
				return;
			}
		}
		...
		fputs(argl, output_file);
		va_end(ap);
		fputc('\n', output_file);
		fflush(output_file);
		if (!one_shot)
			fsync(fileno(output_file));
		if(ftell(output_file) >= LOG_FILE_SIZE)
			ChangeNextLogFile(0);
		return;
	}
...

{
	...
	no_fork = 0;
	use_syscall = 0;
	printf("line:  %d : one_shot=%d \n",__LINE__,one_shot);
	
	/* Parse the command-line. */
	while ((ch = getopt(argc, argv, "c:df:iIk:l:nopsvx2")) != EOF)
		switch((char)ch)
		{
			...
			printf("klogd: main get One-shot mode.\n");
			one_shot = 1;
			break;
		    case 'p':
...
//	use_syscall = 0;
	...
	/* Set output logging level. */
	if ( output_level != (char *) 0 )
	{
		if ( (strlen(output_level) > 1) || \
			 (strchr("12345678", *output_level) == (char *) 0) )
		{
			fprintf(stderr, "klogd: Invalid output logging "
				"level <%s> specified.\n", output_level);
			return(1);
		}
		record_log_level = *output_level - '0';
	}		
	...
	if ( one_shot )
	{
		...
		if ( (logsrc = GetKernelLogSrc()) == kernel )
			...
			LogProcLine();
		fputs("klogd: main one_shot terminate.\n", stderr);
		Terminate();
	}
...
#endif
	printf("line: %s :  %d :  \n",__func__,__LINE__);
	Syslog( LOG_EMERG, "Klogd Starting....\n");
	printf("line:  %d :  \n",__LINE__);
