...
int qca6174TxCommand(const char *txtype)
{
	...
	if (Params->mode == TCMD_CONT_TX_OFF) {
		...
		ALOGE("\n %s() TLV2 CMD_TX add WiFiStandard with rateBw %d\n",
			__func__, gCmd.rateBw);
		if (gCmd.rateBw >= TCMD_RATEBW_HE20 &&
		    gCmd.rateBw < TCMD_RATEBW_OFDMA_HE20)
			tlv2AddParms(2, PARM_WIFISTANDARD, 2);
		//WiFiStandard_OFDMA
		if (gCmd.rateBw >= TCMD_RATEBW_OFDMA_HE20 &&
		    gCmd.rateBw < TCMD_RATEBW_INVALID)
			tlv2AddParms(2, PARM_WIFISTANDARD, 3);
