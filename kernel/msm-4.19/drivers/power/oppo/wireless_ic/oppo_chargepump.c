...
{    
	if (!chip) {
		printk(KERN_ERR "[OPPO_CHG][%s]: chargepump_ic not ready!\n", __func__);
		return;
	}
...
{
	if (!chip) {
		printk(KERN_ERR "[OPPO_CHG][%s]: chip_chargepump not ready!\n", __func__);
		return -EINVAL;
	}
...
#if 0
struct oppo_wpc_operations      *cp_ops = {
...
static const struct of_device_id chargepump_match[] = {
	{ .compatible = "oppo,chgpump-charger"},
	{ },
};
