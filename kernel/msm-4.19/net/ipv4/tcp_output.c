...
extern struct work_struct oppo_ipv4_tcpsynretrans_hook_work;
extern struct oppo_ipv4_tcpsynretrans_hook_struct oppo_ipv4_tcpsynretrans_hook;
#endif /* VENDOR_EDIT */
...
//Asiga@PSW.NW.DATA.2120730, 2019/06/26.
//Add for classify glink wakeup services.
	if (atomic_read(&pcie_rc2_first_msg_ipa) == 1)
...
//Add for classify glink wakeup services.
		if (atomic_read(&tcpsynretrans_hook_boot) == 1) {
			oppo_ipv4_tcpsynretrans_hook.addr = sk->sk_daddr;
			oppo_ipv4_tcpsynretrans_hook.uid = sk->sk_uid;
			schedule_work(&oppo_ipv4_tcpsynretrans_hook_work);
		}
#endif /* VENDOR_EDIT */
