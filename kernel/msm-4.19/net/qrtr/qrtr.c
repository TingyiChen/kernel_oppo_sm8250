...
//Asiga@PSW.NW.DATA.2120730, 2019/06/26.
//Add for classify glink wakeup services.
	if (atomic_read(&pcie_rc2_first_msg_qmi) == 1) {
		...
		if (cb->type == QRTR_TYPE_RESUME_TX) {
			...
			if (!ipc) {
				...
				if (sock_queue_rcv_skb(&ipc->sk, skb)) {
                    //#ifdef VENDOR_EDIT
                    //Yu.Liu@PSW.NW.REG.2728756, 2020/01/06.
                    //Add for adding debug log according to advice from Qcom in case 04392568.
					//pr_err("%s qrtr pkt dropped flow[%d]\n",
					//       __func__, cb->confirm_rx);
                    pr_err("qrtr_node_rx_work: Dropping packet src_node:%d src_port:%d dst_node:%d dst_port: %d confirm_rx: %d\n", cb->src_node, cb->src_port, cb->dst_node, cb->dst_port, cb->confirm_rx);
                    //#endif /* VENDOR_EDIT */
					kfree_skb(skb);
				}
