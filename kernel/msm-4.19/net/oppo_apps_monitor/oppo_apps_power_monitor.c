...
void oppo_app_monitor_fill_retrans_app_info(struct app_monitor_ul_report_msg_st *report_msg)
{
    int  i, j, k;
...
void oppo_app_monitor_send_app_info_report(void)
{
    ...
    oppo_app_monitor_fill_retrans_app_info(report_msg);
...
static inline u32 oppo_app_monitor_find_app_retrans_index(u32 uid_val)
{
    u32 index;
...
void oppo_app_monitor_update_top_retrans_app(struct app_monitor_app_info_st *app_info)
{
    ...
    app_index = oppo_app_monitor_find_app_retrans_index(app_uid);
    //app record exsit, just update it
    if(app_index < TOP_APP_NUM){
...
void oppo_app_monitor_update_retrans_stat(struct app_monitor_app_info_list_node_st *app_info_node,
...
void oppo_app_update_sys_static_info(const struct sk_buff *skb, int send, int retrans)
{
    if(send && retrans){
...
void oppo_app_monitor_update_app_info(struct sock *sk, const struct sk_buff *skb, int send, int retrans)
{
    u32 uid_val;
    u32 hash_index;
    ...
    oppo_app_update_sys_static_info(skb, send, retrans);
    ...
    if(retrans){
        oppo_app_monitor_update_retrans_stat(app_info_node, skb);
        oppo_app_monitor_update_top_retrans_app(&app_info_node->app_info);
    }
