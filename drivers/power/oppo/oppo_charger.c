...
static void oppo_chg_check_rechg_status(struct oppo_chg_chip *chip)
{
	...
	if(chip->wireless_support) {
        ...
        ret_wpc = p922x_wpc_check_recharge();
        if (ret_wpc == 1) {
            chg_err("<linshangbo> wpc decides to recharge!\n");
...
#ifdef SUPPORT_WPC	//for WPC
         chg_err("*****[Batt vol: %d] [Batt max vol: %d] [Batt min vol: %d] [Chg Curr: %d] [Term Vol : S%d,] [Term Curr: %d]*****\n", 
                 chip->batt_volt,
                 chip->batt_volt_max,
                 chip->batt_volt_min,
                 chip->icharging,
...
#ifndef WPC_NEW_INTERFACE
	if(chip->wireless_support){
        ret_wpc = p922x_wpc_check_vbatt_full();
        if (ret_wpc == 0) {
            return false;
        } else if (ret_wpc == 1) {
            return true;
...
int oppo_chg_show_vooc_logo_ornot(void)
{
	...
	if (g_charger_chip->wireless_support && p922x_wpc_get_adapter_type() == CHARGER_SUBTYPE_FASTCHG_SVOOC)
		return 1;
