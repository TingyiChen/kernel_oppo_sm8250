...
#include "oppo_battery_msm8250.h"
...
int smblib_vbus_regulator_enable(struct regulator_dev *rdev)
{
	...
	if (chip->vbatt_num == 2) {
		...
		if (chip->wireless_support) {
			...
			//oppo_set_idt_en_val(0);  //linshangbo debug
			p922x_set_booster_en_val(1);
...
static int oppo_usbtemp_monitor_main(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (((chip->usb_temp_l >= USB_57C) && (chip->usbtemp_volt_l > USB_100C_VOLT))
			...
			count = 0;
			msleep(delay);
		}else if (((chip->usb_temp_l - chip->temperature/10) >= 8)
			|| ((chip->usb_temp_r - chip->temperature/10) >= 8)
			|| (chip->usb_temp_l >= 40) || (chip->usb_temp_r >= 40)) {
			if (dischg_flag == false) {
				if (count == 0) {
		...
		//wait_event_interruptible(oppo_usbtemp_wq, oppo_usb_or_otg_is_present() == true);
		//print usbtemp log every 2 seconds
		log_count++;
		if (log_count == 40) {
